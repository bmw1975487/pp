...
public class MainActivity extends ComponentActivity {
    ...
    private void buildHome() {
        ...
        TextView brand=label("ROOM VISION LIVE 5.1",14,Color.rgb(77,225,238),Gravity.CENTER);brand.setTypeface(Typeface.DEFAULT,Typeface.BOLD);brand.setLetterSpacing(.13f);root.addView(brand,new LinearLayout.LayoutParams(-1,dp(52)));
        TextView title=label("5 живых миров\n+ игровое зрение",32,Color.WHITE,Gravity.CENTER);title.setTypeface(Typeface.DEFAULT,Typeface.BOLD);root.addView(title,new LinearLayout.LayoutParams(-1,dp(120)));
        TextView desc=label("Filament/Vulkan • AGSL • MediaPipe • LiteRT. Камера ищет цветок/растение и подаёт звуковой сигнал при обнаружении.",15,Color.rgb(188,193,203),Gravity.CENTER);LinearLayout.LayoutParams dl=new LinearLayout.LayoutParams(-1,0,1);dl.setMargins(dp(10),dp(18),dp(10),dp(18));root.addView(desc,dl);
        ...
        TextView foot=label("Matrix • Crayon • Blue Pen • ASCII • Gothic",13,Color.rgb(135,239,246),Gravity.CENTER);root.addView(foot,new LinearLayout.LayoutParams(-1,dp(62)));
        setContentView(root);
    }
    ...
    private void showCamera() {
        ...
        TextView status=label("ЗАГРУЗКА MODERN ENGINE STACK…",10,Color.rgb(135,239,246),Gravity.CENTER);bottom.addView(status,new LinearLayout.LayoutParams(-1,dp(34)));
        ...
        for(FilterType type:FilterType.catalog()){Button b=new Button(this);b.setText(type.label);b.setTextSize(11);b.setAllCaps(false);b.setTextColor(Color.WHITE);b.setPadding(dp(14),0,dp(14),0);GradientDrawable cb=new GradientDrawable();boolean world=type==FilterType.MATRIX||type==FilterType.GOTHIC;cb.setColor(world?Color.argb(210,28,42,32):Color.argb(185,25,27,33));cb.setCornerRadius(dp(15));cb.setStroke(dp(1),type==FilterType.MATRIX?Color.rgb(0,255,110):(type==FilterType.GOTHIC?Color.rgb(180,95,90):Color.rgb(100,110,126)));b.setBackground(cb);b.setOnClickListener(v->{if(cameraController!=null)cameraController.setFilter(type);});LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-2,dp(40));lp.setMargins(dp(4),0,dp(4),0);chips.addView(b,lp);}scroll.addView(chips,new HorizontalScrollView.LayoutParams(-2,dp(46)));bottom.addView(scroll,new LinearLayout.LayoutParams(-1,dp(50)));
