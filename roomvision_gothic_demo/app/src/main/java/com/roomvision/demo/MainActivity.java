...
public class MainActivity extends ComponentActivity {
    ...
    private void buildHome() {
        ...
        TextView desc=label("42 действительно разных live-режима по трём исходным приложениям: Pixel Lense, Cartoon Camera HD и Sketch Camera. AI + OpenCV. Без сервера и токенов.",16,Color.rgb(188,193,203),Gravity.CENTER);LinearLayout.LayoutParams dl=new LinearLayout.LayoutParams(-1,0,1);dl.setMargins(dp(10),dp(18),dp(10),dp(18));root.addView(desc,dl);
        ...
        TextView foot=label("Blue Pen • Crayon • Game Boy • Van Gogh • Manga • Oil • Water • Warhol • Cartoon HD",11,Color.rgb(111,116,128),Gravity.CENTER);root.addView(foot,new LinearLayout.LayoutParams(-1,dp(52)));
        setContentView(root);
    }
    ...
    private void showCamera() {
        ...
        TextView mode=label("CARTOON HD",12,Color.WHITE,Gravity.CENTER);mode.setTypeface(Typeface.DEFAULT,Typeface.BOLD);GradientDrawable mb=new GradientDrawable();mb.setColor(Color.argb(175,0,0,0));mb.setCornerRadius(dp(18));mb.setStroke(dp(1),Color.argb(190,74,228,239));mode.setBackground(mb);FrameLayout.LayoutParams ml=new FrameLayout.LayoutParams(dp(240),dp(40),Gravity.TOP|Gravity.CENTER_HORIZONTAL);ml.setMargins(0,dp(30),0,0);root.addView(mode,ml);
        ...
        setContentView(root);cameraController=new NeuralCameraController(this,processed,status,mode);cameraController.setFilter(FilterType.CARTOON_HD);shutter.setOnClickListener(v->{if(cameraController!=null)cameraController.captureCurrentFrame();});cameraController.start();
    }
