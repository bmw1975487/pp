...
public class MainActivity extends ComponentActivity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState){super.onCreate(savedInstanceState);getWindow().setStatusBarColor(Color.rgb(7,8,12));getWindow().setNavigationBarColor(Color.rgb(7,8,12));buildHome();}
    ...
    private void buildHome(){
        cameraMode=false;getWindow().clearFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_VISIBLE);
        ...
        TextView title=label("Живая камера\nв другом виде",34,Color.WHITE,Gravity.CENTER);title.setTypeface(Typeface.DEFAULT,Typeface.BOLD);root.addView(title,new LinearLayout.LayoutParams(-1,dp(110)));
        TextView desc=label("43 live-фильтра из трёх проверенных подходов: OpenCV, GPU-подобные художественные эффекты и on-device AI. Без сервера и токенов.",16,Color.rgb(188,193,203),Gravity.CENTER);LinearLayout.LayoutParams dl=new LinearLayout.LayoutParams(-1,0,1);dl.setMargins(dp(10),dp(18),dp(10),dp(18));root.addView(desc,dl);
        ...
        TextView foot=label("Cartoon • Sketch • Oil • Watercolor • Manga • Thermal • Pixel • AI и другие",11,Color.rgb(111,116,128),Gravity.CENTER);root.addView(foot,new LinearLayout.LayoutParams(-1,dp(52)));
        setContentView(root);
    }
    ...
    private void showCamera(){
        ...
        TextView mode=label("CARTOON",12,Color.WHITE,Gravity.CENTER);mode.setTypeface(Typeface.DEFAULT,Typeface.BOLD);GradientDrawable mb=new GradientDrawable();mb.setColor(Color.argb(175,0,0,0));mb.setCornerRadius(dp(18));mb.setStroke(dp(1),Color.argb(190,74,228,239));mode.setBackground(mb);FrameLayout.LayoutParams ml=new FrameLayout.LayoutParams(dp(220),dp(40),Gravity.TOP|Gravity.CENTER_HORIZONTAL);ml.setMargins(0,dp(30),0,0);root.addView(mode,ml);
        ...
        TextView status=label("ЗАГРУЗКА ФИЛЬТРОВ…",10,Color.rgb(135,239,246),Gravity.CENTER);bottom.addView(status,new LinearLayout.LayoutParams(-1,dp(30)));
        ...
        for(FilterType type:FilterType.catalog()){Button b=new Button(this);b.setText(type.label);b.setTextSize(10);b.setAllCaps(false);b.setTextColor(Color.WHITE);b.setPadding(dp(10),0,dp(10),0);GradientDrawable cb=new GradientDrawable();cb.setColor(type.neural?Color.argb(190,80,25,104):Color.argb(175,25,27,33));cb.setCornerRadius(dp(15));cb.setStroke(dp(1),type.neural?Color.rgb(207,112,255):Color.rgb(92,98,110));b.setBackground(cb);b.setOnClickListener(v->{if(cameraController!=null)cameraController.setFilter(type);});LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-2,dp(38));lp.setMargins(dp(3),0,dp(3),0);chips.addView(b,lp);}scroll.addView(chips,new HorizontalScrollView.LayoutParams(-2,dp(44)));bottom.addView(scroll,new LinearLayout.LayoutParams(-1,dp(48)));
        ...
        FrameLayout.LayoutParams bottomLp=new FrameLayout.LayoutParams(-1,dp(160),Gravity.BOTTOM);root.addView(bottom,bottomLp);

        setContentView(root);cameraController=new NeuralCameraController(this,processed,status,mode);cameraController.setFilter(FilterType.CARTOON);shutter.setOnClickListener(v->{if(cameraController!=null)cameraController.captureCurrentFrame();});cameraController.start();
    }
