...
public class MainActivity extends ComponentActivity {
    ...
    private void buildHome() {
        ...
        TextView brand=label("ROOM VISION GOTHIC 5.3",14,Color.rgb(205,125,112),Gravity.CENTER);brand.setTypeface(Typeface.DEFAULT,Typeface.BOLD);brand.setLetterSpacing(.12f);root.addView(brand,new LinearLayout.LayoutParams(-1,dp(50)));
        ...
        TextView desc=label("Готовый GPUImage Plus 3.2 рендерит Gothic через настоящий CC0 castle-материал Poly Haven, haze, edge и vignette. Камера запускается отдельно от тяжёлых AI-модулей, чтобы режим не падал при старте.",15,Color.rgb(190,193,199),Gravity.CENTER);LinearLayout.LayoutParams dl=new LinearLayout.LayoutParams(-1,0,1);dl.setMargins(dp(8),dp(14),dp(8),dp(18));root.addView(desc,dl);
    ...
    private void showCamera() {
        ...
        TextView status=label("ЗАГРУЗКА GPUIMAGE GOTHIC…",10,Color.rgb(224,163,148),Gravity.CENTER);bottom.addView(status,new LinearLayout.LayoutParams(-1,dp(34)));
