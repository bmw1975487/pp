...
final class NeuralStyleEngine implements AutoCloseable {
    ...
    void initialize() throws IOException {
        ...
        styles.put(FilterType.NEURAL_VAN_GOGH, predictStyle(decodeAsset("styles/van_gogh.jpg")));
        styles.put(FilterType.NEURAL_KANDINSKY, predictStyle(decodeAsset("styles/kandinsky.jpg")));
        styles.put(FilterType.NEURAL_CYBERPUNK, predictStyle(decodeAsset("styles/cyberpunk.jpg")));
    ...
    private void initializeTransformer(boolean preferGpu) throws IOException {
        ...
        if (preferGpu) {
            ...
            try {
                ...
                transformer = null; gpuDelegate = null;
            }
        }
        if (transformer == null) {
            Interpreter.Options o = new Interpreter.Options();
    ...
    private void resolveInputs() {
        contentInputIndex = 0; styleInputIndex = Math.min(1, transformer.getInputTensorCount()-1);
    ...
    private ByteBuffer predictStyle(Bitmap bitmap) {
        try {
            ...
            predictor.run(input,output); output.rewind(); return output;
    ...
    Bitmap processFrame(Bitmap source, FilterType type) {
        ...
        try { return runTransform(source,style); }
            ...
            try { initializeTransformer(false); return runTransform(source,style); }
            catch(Throwable second) { throw new IllegalStateException("Neural inference failed",second); }
        }
    }
    ...
    private Bitmap runTransform(Bitmap source, ByteBuffer styleSource) {
        ...
        int tw,th;
        if(sw>=sh){tw=longSide;th=align8(Math.max(128,Math.round(longSide*(sh/(float)sw))));}
        else {th=longSide;tw=align8(Math.max(128,Math.round(longSide*(sw/(float)sh))));}
        configure(tw,th);
        ByteBuffer content=bitmapToFloatBuffer(source,tw,th);
        ByteBuffer style=styleSource.duplicate().order(ByteOrder.nativeOrder()); style.rewind();
        ...
        inputs[contentInputIndex]=content; inputs[styleInputIndex]=style;
        Map<Integer,Object> outputs=new HashMap<>(); outputs.put(0,output);
        transformer.runForMultipleInputsOutputs(inputs,outputs); output.rewind();
    ...
    private void configure(int w,int h) {
        ...
        transformer.allocateTensors(); configuredWidth=w;configuredHeight=h;
    }

    void setLongSide(int px){longSide=px<=256?256:(px>=512?512:384);configuredWidth=configuredHeight=-1;}
    ...
    private ByteBuffer bitmapToFloatBuffer(Bitmap bitmap,int w,int h){
        Bitmap s=Bitmap.createScaledBitmap(bitmap,w,h,true); int[]p=new int[w*h];s.getPixels(p,0,w,0,0,w,h);
        ByteBuffer b=ByteBuffer.allocateDirect(w*h*12).order(ByteOrder.nativeOrder());
        for(int c:p){b.putFloat(((c>>16)&255)/255f);b.putFloat(((c>>8)&255)/255f);b.putFloat((c&255)/255f);}b.rewind();
        if(s!=bitmap&&!s.isRecycled())s.recycle(); return b;
    }

    private Bitmap floatBufferToBitmap(ByteBuffer b,int w,int h){
        int[]p=new int[w*h];for(int i=0;i<p.length;i++){int r=clamp(Math.round(b.getFloat()*255)),g=clamp(Math.round(b.getFloat()*255)),bl=clamp(Math.round(b.getFloat()*255));p[i]=0xff000000|(r<<16)|(g<<8)|bl;}
        Bitmap out=Bitmap.createBitmap(w,h,Bitmap.Config.ARGB_8888);out.setPixels(p,0,w,0,0,w,h);return out;
    }
    private int clamp(int v){return Math.max(0,Math.min(255,v));}
    private MappedByteBuffer loadModelFile(String name)throws IOException{try(AssetFileDescriptor fd=context.getAssets().openFd(name);FileInputStream in=new FileInputStream(fd.getFileDescriptor());FileChannel ch=in.getChannel()){return ch.map(FileChannel.MapMode.READ_ONLY,fd.getStartOffset(),fd.getDeclaredLength());}}
    private void closeTransformer(){if(transformer!=null)try{transformer.close();}catch(Throwable ignored){} transformer=null;if(gpuDelegate!=null)try{gpuDelegate.close();}catch(Throwable ignored){}gpuDelegate=null;}
    @Override public void close(){if(predictor!=null)try{predictor.close();}catch(Throwable ignored){}predictor=null;closeTransformer();styles.clear();}
}
